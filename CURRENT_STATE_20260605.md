...
- **`execution_issues` field is LLM-inferred, not verified:** The `execution_issues` array in `trade_memory` is generated by the reflection LLM using position metadata at close time — it is not read from Bybit order fills or execution records. Verified against Bybit ground truth (Jun 2026): the LLM invented a scenario for a LINK short ("50% closed at entry price, no partial fired") that was factually wrong — both TP1 and TP2 had actually fired correctly. Treat `execution_issues` as hypothesis/narrative only. Ground truth requires a Bybit order history lookup (`/v5/order/history` or `/v5/execution`) for the trade window.
